...
const W = 800, H = 600, S = 3; // pixel scale
...
// Layout
const LT = 48, LH = 100, GO = 86, DX = 130;
const HT = LT + 4 * LH + 8;

// Physics
const STOCKS = 3, INVT = 2200;
const JV = -300, GR = 920;
const BSP = 200, MSP = 460, SPI = 4, SPINT = 2000;
const EVINT = 18000, EVDUR = 8000;
const EVENTS = ['LOW GRAVITY','SPEED UP','NIGHT MODE','METEOR RAIN','WIND STORM'];
const EV_COLS = [0x9944ff, 0xff4444, 0x2244aa, 0xff6600, 0x22aaaa];
const AI_R = [230, 195], AI_D = [0.88, 0.80];
const OBGAP = 260;

// ─── Chrome Dino Bitmap Data ───────────────────────────
// Body (12 wide × 10 tall), MSB = left
const DB = [0x0FC,0x1FF,0x17F,0x1FF,0x1F0,0x7FE,0xFFF,0xF7F,0xFFF,0x7FE];
...
// Cactus large (6w × 14h)
const CL = [0x0C,0x0C,0x0D,0x0F,0x0D,0x2C,0x3C,0x0C,0x0C,0x0C,0x0C,0x0C,0x0C,0x0C];
// Double cactus (10w × 10h)
const CD = [0x084,0x084,0x2B5,0x3BF,0x2B5,0x084,0x084,0x084,0x084,0x084];
...
function create() {
  ...
  sc.st = {
    ...
    phase:'title', mode:2, players:[], obs:[], spd:BSP, dist:0,
    nextSpd:0, nextEvt:EVINT, curEvt:null, evtEnd:0, sudden:false,
    winner:-1, menu:{cur:0,cd:0,la:0}, freeze:0,
    spawnCd:[0,0,0,0], meteors:[], musicOn:false, hiScores:[],
    lobby:{ready:[false,false,false,false], p2Joined:false, p3Joined:false, p4Joined:false},
  };
  for (let i = 0; i < 4; i++) sc.st.players.push({
    lane:i, x:DX, groundY:LT+i*LH+GO, y:LT+i*LH+GO,
    vy:0, jumping:false, stocks:STOCKS, alive:true,
    invUntil:0, frame:0, animT:0, isAI:false, aiNext:0,
  });
  ...
  loadHS().then(h=>{sc.st.hiScores=h;rfTitleHS(sc)}).catch(()=>{});
}
...
function mkTex(sc) {
  ...
  for (let i = 0; i < 8; i++)
    g.fillRect(i*20+Phaser.Math.Between(0,6), 3+Phaser.Math.Between(0,2), Phaser.Math.Between(4,10), 2);
  ...
  // Meteor
  g.clear();
  g.fillStyle(0xff5500); g.fillCircle(10,10,10);
  g.fillStyle(0xffaa33); g.fillCircle(10,10,5);
  g.fillStyle(0xffee88); g.fillCircle(10,10,2);
  g.generateTexture('mt', 20, 20);

  // Crown
  g.clear(); g.fillStyle(0xffd700);
  g.fillRect(0,12,28,12);
  g.fillRect(0,4,6,10); g.fillRect(11,0,6,14); g.fillRect(22,4,6,10);
  g.fillStyle(0xff3333);
  g.fillRect(2,14,4,4); g.fillRect(12,14,4,4); g.fillRect(22,14,4,4);
  g.generateTexture('crn', 28, 24);

  // Cloud
  g.clear(); g.fillStyle(0xffffff,0.08);
...
function mkBG(sc) {
  // Sky gradient
  const skyColors = [0x0a0a28,0x0f1035,0x141848,0x1c2058,0x283068,0x384878,0x506088,0x687898,0x889098,0xa09888];
  const sh = H / skyColors.length;
  for (let i = 0; i < skyColors.length; i++)
    sc.add.rectangle(W/2, i*sh+sh/2, W, sh+2, skyColors[i]).setDepth(-10);

  // Far mountains
  const mg = sc.add.graphics().setDepth(-8);
  mg.fillStyle(0x1a1a45, 0.7);
  mg.beginPath(); mg.moveTo(0, H);
  for (let x = 0; x <= W; x += 8) {
    const h = 180 + Math.sin(x*0.008)*130 + Math.sin(x*0.019+1)*70 + Math.sin(x*0.034+3)*30;
    mg.lineTo(x, H - h);
  }
  mg.lineTo(W, H); mg.closePath(); mg.fillPath();

  // Near mountains
  mg.fillStyle(0x252555, 0.6);
  mg.beginPath(); mg.moveTo(0, H);
  for (let x = 0; x <= W; x += 6) {
    const h = 130 + Math.sin(x*0.012+2)*100 + Math.sin(x*0.028+1)*50 + Math.sin(x*0.05)*20;
    mg.lineTo(x, H - h);
  }
  mg.lineTo(W, H); mg.closePath(); mg.fillPath();
  ...
  sc.clouds = [];
  for (let i = 0; i < 6; i++) {
    const cl = sc.add.sprite(
      Phaser.Math.Between(0, W),
      Phaser.Math.Between(20, LT + 2*LH),
      'cloud'
    ).setDepth(-5).setAlpha(0.5 + Math.random()*0.3).setScale(1+Math.random()*1.5);
...
// ─── LANES ─────────────────────────────────────────────
function mkLanes(sc) {
  sc.lanes = []; sc.grounds = []; sc.lnOv = []; sc.shadows = [];

  for (let i = 0; i < 4; i++) {
    const ly = LT + i * LH, gy = ly + GO;

    // Lane bg (semi-transparent)
    const lbg = sc.add.rectangle(W/2, ly+LH/2, W, LH, 0x000000, 0.15).setDepth(-3);
    sc.lanes.push(lbg);
    ...
    // Ground TileSprite
    const gnd = sc.add.tileSprite(W/2, gy+5, W, 8, 'gnd').setDepth(2).setAlpha(0.6);
    sc.grounds.push(gnd);
    ...
    // Defeated overlay
    const ov = sc.add.rectangle(W/2, ly+LH/2, W, LH, 0x000000, 0.7).setDepth(18).setVisible(false);
    sc.lnOv.push(ov);
  }
}
...
function mkDinos(sc) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const spr = sc.add.sprite(p.x, p.y, 'dr1').setOrigin(0.5,1).setTint(PC[i]).setDepth(10).setVisible(false);
    sc.dinos.push(spr);
  }
}
...
function mkHUD(sc) {
  ...
  for (let i = 0; i < 4; i++) {
    const x = startX + i * (pw + gap), y = HT + 4;
    ...
    pg.beginPath();
    pg.moveTo(x+sk, y); pg.lineTo(x+pw, y);
    pg.lineTo(x+pw-sk, y+ph); pg.lineTo(x, y+ph);
    ...
    pg.moveTo(x+sk, y); pg.lineTo(x+pw, y);
    pg.lineTo(x+pw-sk, y+ph); pg.lineTo(x, y+ph);
    pg.closePath(); pg.strokePath();

    // Player header tab
    const hdr = sc.add.graphics().setDepth(27);
    hdr.fillStyle(PC[i], 0.8);
    hdr.beginPath();
    hdr.moveTo(x+sk+2, y+1); hdr.lineTo(x+70, y+1);
    hdr.lineTo(x+62, y+18); hdr.lineTo(x+sk-4, y+18);
    hdr.closePath(); hdr.fillPath();

    // Player name
    sc.add.text(x+sk+6, y+2, PN[i], {
      fontFamily: '"Impact","Arial Black",sans-serif',
      fontSize: '14px', color: '#fff', fontStyle: 'bold',
    }).setDepth(28);

    // CPU badge for AI players
    const cpuBadge = sc.add.text(x+pw-42, y+3, i>=2?'CPU':'', {
      fontFamily: '"Impact","Arial Black",sans-serif',
      fontSize: '11px', color: '#888',
    }).setDepth(28);

    // Dino preview sprite in panel
    const preview = sc.add.sprite(x+pw-30, y+ph-6, 'dr1')
      .setOrigin(0.5,1).setTint(PC[i]).setScale(0.7).setAlpha(0.3).setDepth(26);

    // Stock icons
    const icons = [];
    for (let s = 0; s < STOCKS; s++) {
      const si = sc.add.sprite(x+sk+8+s*16, y+32, 'si')
        .setTint(PC[i]).setScale(1.8).setDepth(28);
      icons.push(si);
    }

    // Status text
    const stTxt = sc.add.text(x+sk+60, y+26, 'READY', {
      fontFamily: 'monospace', fontSize: '10px', color: PH[i],
    }).setDepth(28);
  ...
  sc.hud.distTxt = sc.add.text(W/2, HT+ph+16, '00000m', {
    fontFamily: '"Impact","Arial Black",sans-serif',
    fontSize: '22px', color: '#ffffff', fontStyle: 'bold',
  }).setOrigin(0.5).setDepth(28).setAlpha(0.8);

  // Event text
  sc.hud.evtTxt = sc.add.text(W/2, HT+ph+38, '', {
    fontFamily: '"Impact","Arial Black",sans-serif',
    fontSize: '13px', color: '#ff0',
  }).setOrigin(0.5).setDepth(28).setAlpha(0);

  // Sudden death
  sc.hud.sdTxt = sc.add.text(W/2, LT-6, '', {
    fontFamily: '"Impact","Arial Black",sans-serif',
...
function mkTitle(sc) {
  ...
  // Overlay
  c.add(sc.add.rectangle(W/2,H/2,W,H,0x06061a,0.94));

  // Accent lines
  for (let i = 0; i < 4; i++)
    c.add(sc.add.rectangle(W/2-150+i*100, 52, 90, 4, PC[i], 0.8));

  // Subtitle
  c.add(sc.add.text(W/2, 62, 'PLATANUS HACK 26', {
    fontFamily: 'monospace', fontSize: '11px', color: '#555',
  }).setOrigin(0.5));

  // Title "CHROME"
  const t1 = sc.add.text(W/2, 100, 'CHROME', {
    fontFamily: '"Impact","Arial Black",sans-serif',
    fontSize: '44px', color: '#888', fontStyle: 'bold', letterSpacing: 8,
  }).setOrigin(0.5);
  c.add(t1);

  // Title "DINO PARTY"
  const t2 = sc.add.text(W/2, 148, 'DINO PARTY', {
    fontFamily: '"Impact","Arial Black",sans-serif',
    fontSize: '56px', color: '#fff', fontStyle: 'bold', letterSpacing: 4,
  }).setOrigin(0.5);
  c.add(t2);
  sc.tweens.add({
    targets: t2, scaleX:1.02, scaleY:1.02, duration:1400,
  ...
  for (let i = 0; i < 4; i++) {
    ...
    ulG.fillRect(W/2-200+i*100, 175, 95, 4);
  }
  c.add(ulG);

  sc.titleBtns = [];

  const pressStart = sc.add.text(W/2, 330, 'PRESS START TO ENTER LOBBY', {
    fontFamily: '"Impact","Arial Black",sans-serif',
    fontSize: '26px', color: '#ffea00', fontStyle: 'bold',
  }).setOrigin(0.5);
  c.add(pressStart);
  sc.tweens.add({
  ...
  // High scores preview
  c.add(sc.add.text(W/2, 480, 'TOP SCORES', {
    fontFamily: '"Impact","Arial Black",sans-serif',
    fontSize: '12px', color: '#FF4444', fontStyle: 'bold',
  }).setOrigin(0.5));
  sc.titleHSTxt = sc.add.text(W/2, 496, '', {
    fontFamily: 'monospace', fontSize: '11px', color: '#666', align:'center', lineSpacing:2,
  }).setOrigin(0.5,0);
  c.add(sc.titleHSTxt);

  // Footer
  c.add(sc.add.text(W/2, H-14, 'PRESS START BUTTON', {
...
function mkLobby(sc) {
  ...
  c.add(sc.add.text(W/2, 80, 'CHOOSE YOUR FIGHTERS', {
    fontFamily: '"Impact","Arial Black",sans-serif',
    fontSize: '40px', color: '#fff', fontStyle: 'bold',
  }).setOrigin(0.5));

  sc.lobbyCards = [];
  const cw = 170, ch = 200, csk = 15;
  const cs = (W - cw*4 - 24) / 2;
  
  for (let i = 0; i < 4; i++) {
    const cx = cs + i * (cw + 8), cy = 180;
    const g = sc.add.graphics();
    c.add(g);

    const txt = sc.add.text(cx + cw/2, cy + ch - 30, 'CPU', {
      fontFamily: '"Impact","Arial Black",sans-serif',
      fontSize: '20px', color: '#888', align: 'center',
    }).setOrigin(0.5);
    c.add(txt);

    const dpr = sc.add.sprite(cx + cw/2, cy + ch/2, 'dr1')
      .setOrigin(0.5,1).setTint(PC[i]).setScale(3.0).setAlpha(0.4);
    c.add(dpr);

    sc.lobbyCards.push({g, txt, dpr, x:cx, y:cy, w:cw, h:ch, sk:csk, pLabel:PN[i], joined:false});
  }

  c.add(sc.add.text(W/2, H-60, 'JUMP TO JOIN   •   START TO PLAY', {
    fontFamily: '"Impact","Arial Black",sans-serif',
    fontSize: '22px', color: '#ffea00', fontStyle: 'bold',
  }).setOrigin(0.5));
...
function drawAngledBox(g, x, y, w, h, sk, col, alpha, border) {
  ...
  g.beginPath();
  g.moveTo(x+sk, y); g.lineTo(x+w, y);
  g.lineTo(x+w-sk, y+h); g.lineTo(x, y+h);
...
function hiliteTitle(sc) {
  sc.titleBtns.forEach((b, i) => {
    drawAngledBox(b.bg, b.x, b.y, b.w, b.h, b.sk,
      0x1a1a3a, 0.4, 0);
...
function showLobby(sc) {
  ...
  sc.st.phase = 'lobby';
  sc.lobbyCards.forEach((c, i) => {
    c.joined = false; // default to CPU
    c.txt.setText('CPU');
    c.txt.setColor('#888');
    c.dpr.setAlpha(0.4);
    drawAngledBox(c.g, c.x, c.y, c.w, c.h, c.sk, PC[i], 0.1, 0.15);
  });
}
...
function updateLobby(sc, time) {
  ...
  // P1 join logic
  if (anyP(sc, ['P1_1', 'P1_U'])) {
    if (!sc.lobbyCards[0].joined) {
      sc.lobbyCards[0].joined = true;
      changed = true;
    }
  }

  // P2 join logic
  if (anyP(sc, ['P2_1', 'P2_U'])) {
    if (!sc.lobbyCards[1].joined) {
      sc.lobbyCards[1].joined = true;
      changed = true;
    }
  }
  ...
  if (changed) {
    ...
    sc.lobbyCards.forEach((c, i) => {
      if (c.joined) {
        c.txt.setText(`PLAYER ${i+1}\nREADY`);
        c.txt.setColor(PH[i]);
        c.dpr.setAlpha(1.0);
        drawAngledBox(c.g, c.x, c.y, c.w, c.h, c.sk, PC[i], 0.4, 0.8);
      }
    });
  }
...
function rfTitleHS(sc) {
  ...
  sc.titleHSTxt.setText(hs.length
    ? hs.map((e,i) => `${i+1}. ${e.name.padEnd(5)} ${String(e.dist).padStart(5)}m`).join('\n')
    : 'NO SCORES YET');
}
...
function mkPause(sc) {
  ...
  c.add(sc.add.text(W/2, H/2-30, 'PAUSED', {
    fontFamily:'"Impact","Arial Black",sans-serif',
    fontSize:'56px', color:'#fff', fontStyle:'bold',
  }).setOrigin(0.5));
  c.add(sc.add.text(W/2, H/2+30, 'PRESS START TO RESUME', {
...
function mkCtrl(sc) {
  ...
  c.add(sc.add.text(W/2, 80, 'CONTROLS', {
    fontFamily:'"Impact","Arial Black",sans-serif',
    fontSize:'40px', color:'#fff', fontStyle:'bold',
  }).setOrigin(0.5));
  ...
  c.add(sc.add.text(W/2, 160, lines.join('\n'), {
    fontFamily:'monospace', fontSize:'15px', color:'#aaa',
    align:'center', lineSpacing:3,
  }).setOrigin(0.5,0));
  c.add(sc.add.text(W/2,H-20,'PRESS START OR BUTTON TO GO BACK',{
...
function mkLB(sc) {
  ...
  const c = sc.add.container(0,0).setDepth(45);
  c.add(sc.add.rectangle(W/2,H/2,W,H,0x06061a,0.96));
  c.add(sc.add.text(W/2, 80, 'LEADERBOARD', {
    fontFamily:'"Impact","Arial Black",sans-serif',
    fontSize:'40px', color:'#FF4444', fontStyle:'bold',
  }).setOrigin(0.5));
  sc.lbTxt = sc.add.text(W/2, 160, '', {
    fontFamily:'monospace', fontSize:'18px', color:'#ccc',
...
function showLBScreen(sc) {
  const hs = sc.st.hiScores;
  sc.lbTxt.setText(hs.length
    ? hs.map((e,i)=>`${i+1}.  ${e.name.padEnd(6)} ${String(e.dist).padStart(5)}m  ${e.winner}`).join('\n')
    : 'NO SCORES YET');
  sc.ui.title.setVisible(false);
...
function mkEvtBnr(sc) {
  ...
  sc.evtBnrOv = sc.add.rectangle(W/2,H/2,W,H,0x000000,0.5).setDepth(34).setVisible(false);
  sc.evtBnrTxt = sc.add.text(0, 0, '', {
    fontFamily:'"Impact","Arial Black",sans-serif',
...
function showEvtBanner(sc, text, color) {
  ...
  sc.evtBnr.setX(-W);
  sc.tweens.add({
    targets: sc.evtBnr, x: 0, duration: 250, ease: 'Power3',
    onComplete: () => {
      sc.time.delayedCall(1200, () => {
        sc.tweens.add({
          targets: sc.evtBnr, x: W, duration: 250, ease: 'Power3',
...
function mkDefeat(sc) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const t = sc.add.text(W/2, ly, '', {
      fontFamily: '"Impact","Arial Black",sans-serif',
      fontSize: '32px', color: PH[i], fontStyle: 'bold',
    }).setOrigin(0.5).setAngle(-12).setAlpha(0).setDepth(22);
    sc.defTxts.push(t);
  }
}
...
function mkVictory(sc) {
  ...
  sc.vicDino = sc.add.sprite(W/2,300,'dr1').setScale(3).setOrigin(0.5,1);
  c.add(sc.vicDino);

  sc.vicTitle = sc.add.text(W/2,320,'WINNER!',{
    fontFamily:'"Impact","Arial Black",sans-serif',
    fontSize:'52px', color:'#fff', fontStyle:'bold',
  }).setOrigin(0.5);
  c.add(sc.vicTitle);

  sc.vicName = sc.add.text(W/2,375,'',{
    fontFamily:'"Impact","Arial Black",sans-serif',
    fontSize:'30px', color:'#fff', fontStyle:'bold',
  }).setOrigin(0.5);
  c.add(sc.vicName);

  sc.vicStats = sc.add.text(W/2,420,'',{
    fontFamily:'monospace', fontSize:'13px', color:'#888',
    align:'center', lineSpacing:5,
  }).setOrigin(0.5,0);
  c.add(sc.vicStats);

  c.add(sc.add.text(W/2,H-20,'PRESS START TO CONTINUE',{
...
function startGame(sc, time) {
  ...
  sc.st.spd=BSP; sc.st.dist=0; sc.st.nextSpd=time+SPINT;
  sc.st.nextEvt=time+EVINT; sc.st.curEvt=null; sc.st.evtEnd=0;
  sc.st.sudden=false; sc.st.winner=-1; sc.st.freeze=0;
  ...
  const humanCount = sc.st.mode;
  for (let i = 0; i < 4; i++) {
    const p = sc.st.players[i];
    p.stocks=STOCKS; p.alive=true; p.y=p.groundY; p.vy=0;
    p.jumping=false; p.invUntil=0; p.frame=0; p.animT=0; p.aiNext=0;
    p.isAI = !sc.lobbyCards[i].joined;
    ...
    sc.hud.panels[i].stTxt.setText('RUNNING');
...
function mkInput(sc) {
  ...
  const onD = e => {
    ...
    const c = KB[k]; if(!c) return;
    if(!sc.ctrl.held[c]) sc.ctrl.pressed[c]=true;
    sc.ctrl.held[c]=true;
  };
  const onU = e => {
    const k = e.key.length===1?e.key.toLowerCase():e.key;
    const c = KB[k]; if(c) sc.ctrl.held[c]=false;
  };
...
function isH(sc,c){return sc.ctrl.held[c]===true}
function anyP(sc,cs){for(const c of cs){if(sc.ctrl.pressed[c]){sc.ctrl.pressed[c]=false;return true}}return false}
function vA(sc){let a=0;if(isH(sc,'P1_U')||isH(sc,'P2_U'))a--;if(isH(sc,'P1_D')||isH(sc,'P2_D'))a++;return Phaser.Math.Clamp(a,-1,1)}
...
function update(time, delta) {
  ...
  // Clouds scroll
  if (sc.clouds) sc.clouds.forEach(cl => {
    cl.spr.x -= cl.spd * delta/1000;
    if (cl.spr.x < -80) cl.spr.x = W + 80;
  });

  const ph = sc.st.phase;
  if (ph==='title') { updateTitle(sc,time); return; }
  if (ph==='lobby') { updateLobby(sc,time); return; }
  if (ph==='leaderboard'||ph==='controls') {
    if(anyP(sc,['START1','START2','P1_1','P2_1'])){
      sc.ui[ph].setVisible(false); showTitle(sc);
    } return;
  }
  if (ph==='paused') {
    if(anyP(sc,['START1','START2'])){sc.ui.pause.setVisible(false);sc.st.phase='playing';}
    return;
  }
  if (ph==='victory') {
  ...
  if(anyP(sc,['START1','START2'])){sc.st.phase='paused';sc.ui.pause.setVisible(true);return;}

  // Speed up
  if(time>sc.st.nextSpd){sc.st.spd=Math.min(sc.st.spd+SPI,MSP);sc.st.nextSpd=time+SPINT;}
...
function handleInput(sc,time) {
  ...
  if(ps[0].alive && !ps[0].jumping && anyP(sc,['P1_U','P1_1','P1_2'])) doJump(sc,ps[0]);
  if(sc.st.mode >= 2 && ps[1].alive && !ps[1].jumping && anyP(sc,['P2_U','P2_1','P2_2'])) doJump(sc,ps[1]);
}
...
function updateAI(sc,time) {
  for(let i=0;i<4;i++) {
    ...
    for(const o of sc.st.obs) {
      ...
      const dx = o.x - DX;
      if(dx>0 && dx<react) { jump = Math.random()<AI_D[ai<0?0:ai]; break; }
    }
    if(!jump) for(const m of sc.st.meteors) {
      if(Math.abs(m.y-p.groundY)<50 && m.x-DX>0 && m.x-DX<react*0.7) {
        jump = Math.random()<AI_D[ai<0?0:ai]; break;
      }
    }
...
function updatePhys(sc,dt,time) {
  ...
  for(let i=0;i<4;i++) {
    ...
    if(p.jumping) {
      ...
      if(p.y < minY + 39) { p.y = minY + 39; if(p.vy < 0) p.vy = 0; }
      if(p.y >= p.groundY) { p.y = p.groundY; p.vy=0; p.jumping=false; }
    }

    // Wind storm wobble
    const wx = sc.st.curEvt==='WIND STORM' ? -3 + Math.sin(time*0.01)*4 : 0;
    sc.dinos[i].x = p.x + wx;
    sc.dinos[i].y = p.y;
...
function spawnObs(sc,time) {
  ...
  for(let i=0;i<4;i++) {
    ...
    let close = false;
    for(const o of sc.st.obs) if(o.lane===i && o.x>W-OBGAP){close=true;break;}
    if(close) continue;

    const r = Math.random();
    let tex,ow,oh;
    if(r<0.4){tex='cs';ow=5*S;oh=10*S;}
    else if(r<0.75){tex='cl';ow=6*S;oh=14*S;}
    else{tex='cd';ow=10*S;oh=10*S;}
    ...
    const base = sc.st.sudden?700:1300, vary = sc.st.sudden?350:700;
...
function updateObs(sc,dt) {
  ...
  for(let i=sc.st.obs.length-1;i>=0;i--) {
    ...
    if(o.x<-50) {
      ...
      sc.st.obs.splice(i,1); sc.obsSpr.splice(i,1);
    }
  }
}
...
function spawnMeteor(sc) {
  ...
  const spr = sc.add.sprite(W+20, gy-70-Math.random()*30, 'mt').setDepth(11).setScale(1.2);
...
function updateMeteors(sc,dt) {
  for(let i=sc.st.meteors.length-1;i>=0;i--) {
    ...
    m.x+=m.sx*dt; m.y+=m.sy*dt;
    if(sc.metSpr[i]){sc.metSpr[i].x=m.x;sc.metSpr[i].y=m.y;sc.metSpr[i].angle+=200*dt;}
    if(m.x<-30||m.y>H+30){
      if(sc.metSpr[i]) sc.metSpr[i].destroy();
      sc.st.meteors.splice(i,1); sc.metSpr.splice(i,1);
    }
  }
}
...
function checkColl(sc,time) {
  for(let i=0;i<4;i++) {
    ...
    const pw=20,ph=36;
    const px1=p.x-pw/2,px2=p.x+pw/2,py1=p.y-ph,py2=p.y;

    for(const o of sc.st.obs) {
      if(o.lane!==p.lane) continue;
      if(px2>o.x-o.w/2 && px1<o.x+o.w/2 && py2>o.y-o.h && py1<o.y) {
        handleHit(sc,i,time); break;
      }
    }

    if(time>=p.invUntil) for(const m of sc.st.meteors) {
...
function handleHit(sc,pi,time) {
  ...
  sc.tweens.add({targets:sc.dinos[pi],x:p.x-25,duration:120,yoyo:true,ease:'Power2'});

  const si = p.stocks;
  if(si>=0&&si<STOCKS) {
    const icon = sc.hud.icons[pi][si];
    sc.tweens.add({targets:icon,alpha:0,scaleX:2.5,scaleY:2.5,duration:250,onComplete:()=>icon.setVisible(false)});
  }
...
function handleKO(sc,pi,time) {
  ...
  sc.tweens.add({
    targets:sc.defTxts[pi], alpha:1, scaleX:1.2, scaleY:1.2,
    duration:400, ease:'Back.easeOut',
    onComplete:()=>sc.tweens.add({targets:sc.defTxts[pi],alpha:0.5,duration:2000}),
  });
...
function spawnFX(sc, x, y, col) {
  for(let i=0;i<12;i++) {
    ...
    const a = Math.random()*Math.PI*2, d = 20+Math.random()*45;
    sc.tweens.add({
      targets:p,x:x+Math.cos(a)*d,y:y+Math.sin(a)*d,
      alpha:0,scaleX:0.15,scaleY:0.15,angle:Phaser.Math.Between(-180,180),
      duration:200+Math.random()*250,onComplete:()=>p.destroy(),
    });
  }
  // Smoke puffs
  for(let i=0;i<4;i++){
    const r=sc.add.circle(x-8-i*6,y-4,5+i*2,0x888888,0.4).setDepth(15);
    sc.tweens.add({targets:r,x:x-25-i*10,alpha:0,scaleX:2.5,scaleY:2.5,duration:350+i*80,onComplete:()=>r.destroy()});
  }
}
...
function checkSudden(sc,time) {
  ...
  if(alive.length===2 && alive.every(p=>p.stocks===1) && !sc.st.sudden) {
    sc.st.sudden = true;
    sc.st.spd = Math.max(sc.st.spd, 380);
    sc.hud.sdTxt.setText('⚡ SUDDEN DEATH ⚡');
    sc.tweens.add({targets:sc.hud.sdTxt,alpha:1,duration:200,yoyo:true,repeat:5,hold:150,
      onComplete:()=>sc.tweens.add({targets:sc.hud.sdTxt,alpha:0.8,duration:500,yoyo:true,repeat:-1})
    });
    sfx(sc,'event');
...
function showVictory(sc) {
  ...
  sc.tweens.add({targets:sc.vicDino,scaleX:3,scaleY:3,alpha:1,duration:500,ease:'Back.easeOut'});
  sc.vicCrn.setAlpha(0);
  sc.tweens.add({targets:sc.vicCrn,alpha:1,y:165,duration:350,delay:350,ease:'Bounce.easeOut'});
  sc.tweens.add({targets:sc.vicGlow,scaleX:1.3,scaleY:1.3,alpha:0.06,duration:1000,yoyo:true,repeat:-1});
  sfx(sc,'victory');

  if(wi>=0) {
    const entry = {name:PN[wi]+(wi>=sc.st.mode?' CPU':''), winner:PN[wi], dist:Math.floor(sc.st.dist/10), savedAt:new Date().toISOString().slice(0,10)};
    persistHS(entry).then(h=>{sc.st.hiScores=h}).catch(()=>{});
  }
}

function backToTitle(sc) {
  sc.ui.victory.setVisible(false);
  if(sc.obsSpr){sc.obsSpr.forEach(s=>s.destroy());sc.obsSpr=[];}
  if(sc.metSpr){sc.metSpr.forEach(s=>s.destroy());sc.metSpr=[];}
  sc.st.obs=[];sc.st.meteors=[];
  showTitle(sc);
}
...
function updateEvts(sc,time,dt) {
  ...
  if(sc.st.curEvt==='METEOR RAIN') {
    if(!sc.st.nextMet||time>sc.st.nextMet){spawnMeteor(sc);sc.st.nextMet=time+500+Math.random()*400;}
  }
}
...
function trigEvt(sc,time) {
  ...
  if(evt==='NIGHT MODE') sc.lanes.forEach(l=>l.setFillStyle(0x000020,0.55));
...
function endEvt(sc) {
  ...
  if(evt==='NIGHT MODE'){
    sc.lanes.forEach(l=>l.setFillStyle(0x000000,0.15));
    if(sc.obsSpr) sc.obsSpr.forEach(s=>{if(s.active)s.clearTint()});
  }
...
function updateAnims(sc,time) {
  for(let i=0;i<4;i++) {
    const p = sc.st.players[i];
    if(!p.alive) continue;
    if(p.jumping) { sc.dinos[i].setTexture('dj'); }
    else {
      const rate = Math.max(70, 200 - sc.st.spd*0.3);
      if(time>p.animT){p.frame=1-p.frame;p.animT=time+rate;}
...
function updateGnd(sc,dt,time) {
  ...
  for(let i=0;i<4;i++) if(sc.st.players[i].alive) sc.grounds[i].tilePositionX+=scroll;

  // Speed lines during SPEED UP
  if(sc.st.curEvt==='SPEED UP'&&Math.random()<0.35) {
    const ly = LT+Math.random()*(LH*4);
    const line = sc.add.rectangle(W+20,ly,50+Math.random()*80,1,0xffffff,0.1).setDepth(5);
    sc.tweens.add({targets:line,x:-100,duration:180,onComplete:()=>line.destroy()});
  }
  // Wind particles
  if(sc.st.curEvt==='WIND STORM'&&Math.random()<0.5) {
    const ly = LT+Math.random()*(LH*4);
    const dot = sc.add.circle(W+10,ly,2,0x88aacc,0.35).setDepth(5);
    sc.tweens.add({targets:dot,x:-20,y:ly+Math.random()*20-10,duration:400+Math.random()*250,onComplete:()=>dot.destroy()});
  }
}

// ─── HUD REFRESH ───────────────────────────────────────
function rfHUD(sc,time) {
  sc.hud.distTxt.setText(`${String(Math.floor(sc.st.dist/10)).padStart(5,'0')}m`);
...
// ─── AUDIO ─────────────────────────────────────────────
function gCtx(sc){try{return sc.sound&&sc.sound.context?sc.sound.context:null}catch(_){return null}}

function sfx(sc,type){
  try{
    const ctx=gCtx(sc);if(!ctx) return;
    ...
    if(type==='jump'){
      ...
      o2.frequency.setValueAtTime(100,t);o2.frequency.exponentialRampToValueAtTime(35,t+0.4);
      g2.gain.setValueAtTime(0.18,t);g2.gain.exponentialRampToValueAtTime(0.001,t+0.45);
      ...
      o.type='square';
      o.frequency.setValueAtTime(440,t);o.frequency.setValueAtTime(660,t+0.1);
      o.frequency.setValueAtTime(880,t+0.2);
      g.gain.setValueAtTime(0.1,t);g.gain.exponentialRampToValueAtTime(0.001,t+0.35);
      o.start(t);o.stop(t+0.35);
    }else if(type==='victory'){
      o.type='square';
      o.frequency.setValueAtTime(440,t);o.frequency.setValueAtTime(554,t+0.15);
      o.frequency.setValueAtTime(660,t+0.3);o.frequency.setValueAtTime(880,t+0.45);
      g.gain.setValueAtTime(0.12,t);g.gain.setValueAtTime(0.12,t+0.4);
...
function startMusic(sc){
  ...
  try{
    ...
    const dly=ctx.createDelay(2),dlf=ctx.createGain();
    dly.delayTime.value=0.375;dlf.gain.value=0.18;
    dly.connect(dlf);dlf.connect(dly);dlf.connect(m);

    const pf=ctx.createBiquadFilter();pf.type='lowpass';
    pf.frequency.value=550;pf.Q.value=1.2;pf.connect(m);pf.connect(dly);

    const lfo=ctx.createOscillator(),lfg=ctx.createGain();
    lfo.frequency.value=0.06;lfg.gain.value=300;
    lfo.connect(lfg);lfg.connect(pf.frequency);lfo.start();

    [[82.41,0,'sawtooth'],[82.41,7,'sawtooth'],[98,-5,'triangle'],[123.47,3,'triangle'],[146.83,-3,'triangle']].forEach(([f,d,tp])=>{
      const o=ctx.createOscillator(),g=ctx.createGain();
      o.type=tp;o.frequency.value=f;o.detune.value=d;
      g.gain.value=0.018;o.connect(g);g.connect(pf);o.start();
    });

    const ARP=[164.81,196,246.94,293.66,329.63,392,329.63,293.66,246.94,196];
    const ST=0.19,AL=ARP.length*ST;
    function sA(t0){
      ARP.forEach((f,i)=>{
        const t=t0+i*ST;
        const o=ctx.createOscillator(),g=ctx.createGain();
        o.type='square';o.frequency.value=f;o.connect(g);g.connect(m);g.connect(dly);
        g.gain.setValueAtTime(0.001,t);g.gain.linearRampToValueAtTime(0.035,t+0.015);
        g.gain.exponentialRampToValueAtTime(0.0001,t+ST*0.6);
        o.start(t);o.stop(t+ST*0.65);
      });
    ...
    function sB(t){
      const o=ctx.createOscillator(),g=ctx.createGain();
      o.type='sine';o.frequency.value=55;o.connect(g);g.connect(m);
      g.gain.setValueAtTime(0.26,t);g.gain.exponentialRampToValueAtTime(0.001,t+0.25);
    ...
    function sTk(t){
      const o=ctx.createOscillator(),g=ctx.createGain();
      o.type='square';o.frequency.value=2800;o.connect(g);g.connect(m);
      g.gain.setValueAtTime(0.012,t);g.gain.exponentialRampToValueAtTime(0.0001,t+0.015);
      o.start(t);o.stop(t+0.02);
      sc.time.delayedCall(TK*1000,()=>sTk(t+TK));
    }

    const t0=ctx.currentTime+0.3;
    sA(t0);sB(t0);sTk(t0+TK/2);
  }catch(_){}
}
...
function gS(){
  ...
  return {
    async get(k){try{const r=localStorage.getItem(k);return r===null?{found:false,value:null}:{found:true,value:JSON.parse(r)}}catch{return{found:false,value:null}}},
    async set(k,v){localStorage.setItem(k,JSON.stringify(v))},
  };
}
async function loadHS(){const r=await gS().get(STORE_KEY);if(!r.found||!Array.isArray(r.value))return[];return r.value.filter(e=>e&&typeof e.dist==='number').slice(0,MAX_HS)}
async function persistHS(entry){const ex=await loadHS();const n=ex.concat(entry).sort((a,b)=>b.dist-a.dist).slice(0,MAX_HS);await gS().set(STORE_KEY,n);return n}
